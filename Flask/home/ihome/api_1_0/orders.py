...
def save_order():
    ...
    try:
        ...
        #计算预订的天数
        days = (end_date - start_date).days +1  #.days 得到的是日
    ...
    try:
        ...
        current_app.logger.error(e)
        return jsonify(errno=RET.DBERR, errmsg="检查错误，请稍后重试")
    if count > 0:
...
# api/v1.0/user/orders?role=custom role=landlord   顾客 房东
@api.route("/user/orders",methods=["GET"])
@login_required
def get_user_orders():
    """  查询用户的订单信息"""
    user_id = g.user_id
    #用户的身份，用户想要查询作为房客预订别人房子的订单，还是要作为房东查询别人预订房子的订单
...
def save_order_comment(order_id):
    ...
    try:
        ...
        db.session.rollback()
        return jsonify(errno= RET.DBERR,errmsg="操作失败")
    #因为房屋详情中有订单的评价信息，为了让最新的评价信息展示在房屋详情，所以删除redis中有关本订单房屋的详情
    try:
